...
class RiskManager:
    ...
    def __init__(self, hourly_rate: float = DEFAULT_HOURLY_RATE):
        ...
        if isinstance(hourly_rate, bool) or not isinstance(hourly_rate, (int, float)):
            raise TypeError("hourly_rate must be a finite positive number")
        if not math.isfinite(hourly_rate) or hourly_rate <= 0:
            raise ValueError("hourly_rate must be a finite positive number")
    ...
    def generate_risk_heat_map(self, risks: list[dict[str, Any]],
        ...
        for risk in risks:
            ...
            likelihood = self._likelihood_percentage(exploitability)
    ...
    @staticmethod
    def _likelihood_percentage(exploitability: Any) -> float:
        """Normalize supported exploitability payloads to a 0-100 percentage."""
        if not isinstance(exploitability, dict):
            return 50.0

        if 'exploitability_score' in exploitability:
            raw_score = exploitability['exploitability_score']
            scale = 10.0
        elif 'score' in exploitability:
            raw_score = exploitability['score']
            scale = 1.0
        else:
            return 50.0
